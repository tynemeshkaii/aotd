-- Lock down function EXECUTE grants. PostgreSQL grants EXECUTE on functions
-- to PUBLIC by default, so revoke PUBLIC first, then grant explicitly.

revoke execute on function public.discovery_album_rows(uuid) from public;
revoke execute on function public.ensure_recommendation_atomic(uuid, uuid, date, integer, jsonb, boolean, text, text) from public;
revoke execute on function public.find_users_due_for_compute(integer, integer) from public;
revoke execute on function public.resolve_user_compute_context(uuid) from public;
...
revoke execute on function public.record_external_api_circuit_failure(text, text, integer, text, integer) from public;
revoke execute on function public.prune_external_api_request_log() from public;
revoke execute on function public.try_start_library_sync(uuid, timestamptz, interval) from public;

revoke execute on function public.get_current_pick(uuid) from public;
revoke execute on function public.get_discoveries(uuid, integer, integer) from public;
revoke execute on function public.get_discovery_detail(uuid, uuid) from public;
revoke execute on function public.save_album_rating(uuid, uuid, integer, text) from public;
revoke execute on function public.safe_profile_timezone(text) from public;
