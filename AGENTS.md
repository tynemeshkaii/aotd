...
- Palettes are resolved at runtime by `EditorialThemeProvider` / `useEditorialPalette`. `components/skins/shared/skinStyles.ts` exports `dayPalette` (warm paper/ink) and `nightPalette` (evening dark); `editorialColors` is a back-compat alias for `dayPalette`. Each palette carries an `isDark` flag — drive chrome (status-bar style, etc.) from `palette.isDark`, never from a hardcoded hex compare like `palette.paper === '#17120f'`.
- Edition is Day / Night / System. The Profile "Edition" segmented control (`EditionToggle`) sets it; the choice persists to AsyncStorage under `editorial-edition`; `System` live-follows the OS color scheme via `useColorScheme()`. The provider mounts in `app/_layout.tsx` inside the boot gates so the splash reads the resolved palette. Spotify green stays green in both editions.
- ShareCard always captures the **Day** palette regardless of the active in-app edition (shared PNGs live outside the app's dark context and must stay legible). The off-screen capture target forces `dayPalette`; do not let it inherit night.
...
- The Home hero masthead rule "ink-bleeds" in from the left on screen focus (`EditorialMasthead` `reveal` prop, Reanimated width reveal — pure Reanimated, not Skia, so the Home hero still works in Expo Go). Gated through `useAccentFlow().reduceMotion`: when Reduce Motion is on the rule is instant/full. Only Home enables `reveal`; the same masthead on Archive/Profile/Recap stays static (`reveal` defaults false). This stays within flowing-accent scarcity — do not enable it elsewhere.
